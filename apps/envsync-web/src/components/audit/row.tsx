    ...
    >
      <td className="py-3 px-4">
        <div className="flex items-center space-x-2">
          <div className="size-8 bg-zinc-800 rounded-full flex items-center justify-center overflow-hidden flex-shrink-0">
            <Avatar className="w-full h-full rounded-none overflow-hidden">
              ...
              />
              <AvatarFallback className="bg-inherit text-zinc-300 text-xs">
                {log.user_name
                  .split(" ")
                  .map((n) => n[0])
                  .join("")
      ...
      <td className="py-3 px-4">
        <div className="flex items-center space-x-2">
          <span className="text-zinc-500">{log.resourceIcon}</span>
          <div>
            <div className="text-sm text-zinc-200">
              {log.project || log.environment || log.action}
            </div>
            {!!log.resource_type && (
              <div className="text-[11px] text-zinc-500">{log.resource_type}</div>
            )}
          </div>
        </div>
      </td>
      ...
      <td className="py-3 px-4">
        <div>
          ...
          {log.details && (
            <div className="text-[11px] text-zinc-500 mt-0.5 max-w-xs truncate">
              {log.details}
            </div>
          )}
        </div>
