...
export class EnvStorePiTService {
	...
	public static getEnvStorePiTs = async ({
		...
		let historyQuery = db
			...
			.select(eb => [
				"env_store_pit.id",
				"env_store_pit.org_id",
				"env_store_pit.app_id",
				"env_store_pit.env_type_id",
				...
				eb
					.selectFrom("env_store_pit_change_request")
					.select(db.fn.count<number>("env_store_pit_change_request.id").as("count"))
					.whereRef("env_store_pit_change_request.env_store_pit_id", "=", "env_store_pit.id")
					.as("changes_count"),
			])
		...
		let totalCountQuery = db
			.selectFrom("env_store_pit")
			.select(db.fn.count<number>("env_store_pit.id").as("count"))
			.where("env_store_pit.org_id", "=", org_id)
			.where("env_store_pit.app_id", "=", app_id)
			.where("env_store_pit.env_type_id", "=", env_type_id);

		if (from_created_at) {
			historyQuery = historyQuery.where("env_store_pit.created_at", ">=", from_created_at);
			totalCountQuery = totalCountQuery.where("env_store_pit.created_at", ">=", from_created_at);
		}

		if (to_created_at) {
			const toCreatedAtExclusive = createExclusiveUpperBound(to_created_at);
			historyQuery = historyQuery.where("env_store_pit.created_at", "<", toCreatedAtExclusive);
			totalCountQuery = totalCountQuery.where("env_store_pit.created_at", "<", toCreatedAtExclusive);
		}

		const [pits, totalCount] = await Promise.all([
			historyQuery.orderBy("env_store_pit.created_at", "desc").limit(per_page).offset((page - 1) * per_page).execute(),
	...
	public static getEnvsTillPiTId = async ({
		...
		const allChanges = await db
			...
			.where("env_store_pit.app_id", "=", app_id)
			.where("env_store_pit.env_type_id", "=", env_type_id)
			.where("env_store_pit.created_at", "<", createExclusiveUpperBound(targetPiT.created_at))
			.orderBy("env_store_pit.created_at", "asc")
			.orderBy("env_store_pit_change_request.created_at", "asc")
			.execute();
	...
	public static getEnvsTillTimestamp = async ({
		...
		const allChanges = await db
			...
			.where("env_store_pit.app_id", "=", app_id)
			.where("env_store_pit.env_type_id", "=", env_type_id)
			.where("env_store_pit.created_at", "<", createExclusiveUpperBound(timestamp))
			.orderBy("env_store_pit.created_at", "asc")
			.orderBy("env_store_pit_change_request.created_at", "asc")
			.execute();
	...
	public static getEnvDiff = async ({
		...
		const allChanges = await db
			...
			.where("env_store_pit.app_id", "=", app_id)
			.where("env_store_pit.env_type_id", "=", env_type_id)
			.where("env_store_pit.created_at", "<", createExclusiveUpperBound(toPiT.created_at))
			.orderBy("env_store_pit.created_at", "asc")
			.orderBy("env_store_pit_change_request.created_at", "asc")
			.execute();
	...
	public static getEnvDiffByTimestampRange = async ({
		...
		const allChanges = await db
			...
			.where("env_store_pit.app_id", "=", app_id)
			.where("env_store_pit.env_type_id", "=", env_type_id)
			.where("env_store_pit.created_at", "<", createExclusiveUpperBound(to_timestamp))
			.orderBy("env_store_pit.created_at", "asc")
			.orderBy("env_store_pit_change_request.created_at", "asc")
			.execute();
