...
interface SeededEnvPitScenario {
	seed: SeedOrgResult;
	appId: string;
	envTypeId: string;
...
async function seedScenario(): Promise<SeededEnvPitScenario> {
	await cleanupDB();
	resetFGA();
	resetVaultStore();
	...
	const app = await seedApp(seed.org.id, { name: "PiT Range Env App" });
	const envType = await seedEnvType(seed.org.id, app.id, { name: "pit-range-env" });
...
describe("Environment PiT range APIs", () => {
	...
	test("history supports lower bound, upper bound, both bounds, and filtered totalPages", async () => {
		...
		const lowerBoundRes = await testRequest("/api/env/history", {
			method: "POST",
			token: seed.masterUser.token,
			body: {
				app_id: appId,
		...
		const upperBoundRes = await testRequest("/api/env/history", {
			method: "POST",
			token: seed.masterUser.token,
			body: {
				app_id: appId,
		...
		const boundedRes = await testRequest("/api/env/history", {
			method: "POST",
			token: seed.masterUser.token,
			body: {
				app_id: appId,
		...
		const invalidRangeRes = await testRequest("/api/env/history", {
			method: "POST",
			token: seed.masterUser.token,
			body: {
				app_id: appId,
	...
	test("timestamp-range diff handles added, modified, deleted, mixed, and no-op ranges", async () => {
		...
		const addedRes = await testRequest("/api/env/diff/timestamp-range", {
			method: "POST",
			token: seed.masterUser.token,
			body: {
				app_id: appId,
		...
		const modifiedRes = await testRequest("/api/env/diff/timestamp-range", {
			method: "POST",
			token: seed.masterUser.token,
			body: {
				app_id: appId,
		...
		expect(modifiedBody.modified).toEqual([{ key: "A", old_value: "1", new_value: "2" }]);

		const deletedRes = await testRequest("/api/env/diff/timestamp-range", {
			method: "POST",
			token: seed.masterUser.token,
			body: {
				app_id: appId,
		...
		expect(deletedBody.deleted).toEqual([{ key: "B", value: "1" }]);

		const mixedRes = await testRequest("/api/env/diff/timestamp-range", {
			method: "POST",
			token: seed.masterUser.token,
			body: {
				app_id: appId,
		...
		expect(mixedBody.added).toEqual([{ key: "C", value: "1" }]);
		expect(mixedBody.modified).toEqual([{ key: "A", old_value: "1", new_value: "2" }]);
		expect(mixedBody.deleted).toEqual([{ key: "B", value: "1" }]);

		const noopRes = await testRequest("/api/env/diff/timestamp-range", {
			method: "POST",
			token: seed.masterUser.token,
			body: {
				app_id: appId,
